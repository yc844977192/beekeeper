...
<div class="info_table">
    <table class="table table-striped table-hover">
        ...
        <tbody>
        @foreach($offererLists as $key => $value)
            <tr class="bt-commodity-details">
                <td>{{$value['offerer_name']}}</td>
                <td>{{$value['offerer_mobile']}}</td>
                <td>{{$value['created_at']}}</td>
                <td><a href="/admin/offerer/{{$value['id']}}">查看详情</a></td>
            </tr>
        @endforeach
        </tbody>
    </table>
...
<script>
    ...
    function btn2(pageNum) {
        var requestUrl = '/admin/commodity/offererAjax';
